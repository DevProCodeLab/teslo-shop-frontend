          ...
          {menuItems.map((item, index) => {
            ...
            return (
              <li key={index}>
                <Link
                  to={item.to || '/admin'}
                  className={`flex items-center space-x-3 px-3 py-2 rounded-lg transition-all duration-200 group ${isActiveRoute(item.to || '/xxxx') ? 'bg-blue-50 text-blue-600 border-r-2 border-blue-600'
        ...
        <div className="p-4 border-t border-gray-200">
          <div className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
            <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold">
              {!user ? 'AN' : getInitials(user.fullName)}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 truncate">{!user ? 'Anonymus' : user.fullName}</p>
              <p className="text-xs text-gray-500 truncate">{!user ? 'Anonymus' : user.email}</p>
            </div>
          </div>
        </div>
      )}
